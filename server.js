...
async function handle_invoice_response(request, response)
{
  let invoice_document = new pdfkit;

  // Set up company information.
  const companyInfo = {
      name: "NIU Automative Parts",
      email: "www.niuautomotive.com"
  };
  ...
  const invoiceDetails = {
      id: request.params.iid.substring(0, 14),
      date: "4/12/2024",
      items: [
  ...
  let order_details = await fetch_order(request.params.iid);
  //console.log(order_details);
  let connection = await set_connection();

  invoiceDetails.date = unix_to_time(order_details[0].DateAndTime);
  customerInfo.name = order_details[0].first_name + " " + order_details[0].last_name;
  customerInfo.street = order_details[0].Address_line1;
  customerInfo.city = order_details[0].city;
  customerInfo.state = order_details[0].customer_state;
  customerInfo.zip = order_details[0].postal_code;
  customerInfo.email = order_details[0].email;
  customerInfo.phone = "N/A";

  for (let i = 0; i < order_details.length; ++i)
  {
    let line = order_details[i];
    let details = await connection
      .query(`SELECT * FROM parts WHERE number = ${line.part_number}`)
      .then(([rows, fields]) => { return rows; });

    invoiceDetails.items.push({
      itemnum: line.part_number,
  ...
  invoice_document.font('Helvetica')
      .fontSize(10)
      .text(`${invoiceDetails.id}`, iidX, iidY, { width: textWidth });
  ...
  invoice_document.font('Helvetica')
      .fontSize(12)
      .fillColor(`black`)
      .text("Thank you for choosing NIU Automative Parts!",
          (pageWidth - invoice_document.widthOfString("Thank you for choosing NIU Automative Parts!")) / 2, 750);
...
app.get('/api/invoice/:iid', (request, response) => {
    handle_invoice_response(request, response);
});

async function handle_packlist_request(request, response)
{
  ...
  let packlistInfo = {
      id: request.params.pid.substr(0, 12),
  ...
  let orderInfo = {
      customerid: "1",
      date: "4/8/2024",
      ordernum: request.params.pid.substr(0, 12)
  };
  ...
  let billToInfo = shipToInfo;

  let order_details = await fetch_order(request.params.pid);
  //console.log(order_details);
  let connection = await set_connection();

  //orderInfo.customerid = order_details[0].customer_ID;

  shipToInfo.name = order_details[0].first_name + " " + order_details[0].last_name;
  shipToInfo.street = order_details[0].Address_line1;
  shipToInfo.city = order_details[0].city;
  shipToInfo.state = order_details[0].customer_state;
  shipToInfo.zip = order_details[0].postal_code;
  shipToInfo.phone = "N/A";
  ...
  for (let i = 0; i < order_details.length; ++i)
  {
    ...
    let details = await connection
      ...
      packlistInfo.items.push({
      itemnum: line.part_number,
      description: details[0].description,
      orderqty: line.quantity,
      shipqty: line.quantity,
      price: parseFloat(line.quantity) * parseFloat(details[0].weight),
  ...
  packlist_document.font('Helvetica-Bold')
      .fontSize(20)
      .text(`NIU Automative Parts`, 25, 20);
  ...
  // Web address.
  packlist_document.text(`www.niuautomotive.com`, 25, 57)
      .moveDown();
...
app.get('/api/packlist/:pid', (request, response) => {
  handle_packlist_request(request, response)
});

async function handle_shiplabel_request(request, response)
{
    ...
    const sender = {
        name: `NIU Automotive`,
        address: `1234 University Avenue`,
        city: `Dekalb`,
        state: `IL`,
        postalCode: `60115`
    }
    ...
    let packageInfo = {
        trackingNumber: "1234-1234-5678-5678", // Assuming the tracking number is passed as a parameter
        weight: 999,
        routingNumber: '9405 5112 9837 0132 0951 98'
    };
    let connection = await set_connection();
    let order_details = await fetch_order(request.params.slabel);
    recipient.name = order_details[0].first_name + " " + order_details[0].last_name;
    recipient.addressLine1 = order_details[0].Address_line1;
    recipient.addressLine2 = order_details[0].Address_line2;
    recipient.city = order_details[0].city;
    recipient.state = order_details[0].customer_state;
    recipient.zip = order_details[0].postal_code;
    ...
    for (let i = 0; i < order_details.length; ++i)
    {
      ...
      total_weight += details[0].weight * line.quantity;
    }

    packageInfo.weight = total_weight;
...
app.get('/api/shiplabel/:slabel', (request, response) => {
  handle_shiplabel_request(request, response);
});
